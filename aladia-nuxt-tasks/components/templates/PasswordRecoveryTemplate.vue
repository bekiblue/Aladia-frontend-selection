...
class="flex items-center justify-center min-h-screen bg-black "  >
      ...
      :show-back-icon="showPasswordRecoverySection"
    />
    <LoginBanner
      title="Add a new password"
      subtitle="Your account uses
google Sign-in.
Add a new password."
      :imgSrc="googleImgSrc"
    />
    ...
    <div class="flex flex-1 flex-col px-6">
      <div v-if="showPasswordRecoverySection" class="leading-none text-white/70">
        <div class="text-sm font-semibold text-white/70">
          Recovery link sent to:
        </div>
        <div class="text-xs font-light italic">{{ email }}</div>
      </div>

      <div
        v-if="showPasswordRecoverySection"
        class="flex flex-1 items-center justify-between gap-6"
      >
        <img src="/assets/img/aladia.png" class="h-20 w-20 object-cover" />
        <div class="flex flex-col items-center gap-3">
          <font-awesome icon="arrow-right" class="text-xl" />
          <img
      ...
      <div v-else class="flex flex-1 flex-col gap-2 items-center justify-center">
      ...
      <Button
        :label="
          showPasswordRecoverySection ? 'Send Link' : 'Create New Password'
        "
        data-cy="btn"
...
const props = defineProps({
  email: {
    type: String,
    default: "bereket.tsegaye@gmail.com",
  },
  showPasswordRecoverySection: { type: Boolean, default: true },
});
...
const handleClick = () => {
  ...
  setTimeout(() => {
    ...
    if (props.showPasswordRecoverySection) {
      appState.showPasswordSetField();
    } else {
      appState.login();
    }
  }, 2000);
